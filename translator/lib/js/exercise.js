  ...
  static replaceHints () {
    ...
    const findQuestion = (element) => {
      ...
      while (sibling) {
        ...
        if (sibling && sibling.className.match(/\bquestion\b/)) {
          return sibling;
        }
      }
      return null;
    ...
    // /* _questionId */
    for (const [, dependentElements] of questionClusters) {
      for (let dependentElementIndex = 0; dependentElementIndex < dependentElements.length; dependentElementIndex++) {
        ...
        if (dependentElementIndex === 0) {
          dependentElement
            ...
            .addEventListener('click', makeClickListener(dependentElements.slice(1)));
        } else {
